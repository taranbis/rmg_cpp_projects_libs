...
int main()
{
    ...
    while (true)
    {
        ...
        { // the forklift is moving => x,y vary
        ...
        { // the forklift is moving => z varies
...
void publish(std::string msg)
{
    ...
    if (ss >> timestamp >> id >> x >> y >> z)
    {
        ...
        float    distance = (x - previousData.x) * (x - previousData.x) +
                         (y - previousData.y) * (y - previousData.y);
        float xySpeed = sqrt(distance) * 1000 / time;
        std::cout << "Speed is: " << xySpeed << "[m/s], which is "
                  << xySpeed * 3.6 << "[km/h]" << std::endl;
        float zSpeed = sqrt(z - previousData.z) * 1000 / time;
        std::cout << "Lifting speed is: " << zSpeed << "[m/s]" << std::endl;
        totalDistance += distance;
        std::cout << "Total distance travelled by the forklift: " << totalDistance
                  << "[m]" << std::endl;
