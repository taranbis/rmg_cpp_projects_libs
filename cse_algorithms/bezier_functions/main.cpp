...
int main(int argc, char *argv[])
{
    ...
    SDL_Window *window = SDL_CreateWindow("Bezier Functions", 
    ...
    std::unique_ptr<Point> c1(new Point{250, 50});  controlPoints.push_back(c1.get());
    std::unique_ptr<Point> c2(new Point{500, 400}); controlPoints.push_back(c2.get());

    //Create anchors
    std::unique_ptr<Point> a0 (new Point{50, 200});  anchorPoints.push_back(a0.get());
    std::unique_ptr<Point> a3 (new Point{750, 250}); anchorPoints.push_back(a3.get());

    std::merge(controlPoints.begin(), controlPoints.end(), anchorPoints.begin(), anchorPoints.end(), std::back_inserter(points));
    ...
    std::vector<Point> bezierCurve; bezierCurve.reserve(10000);
    ...
    while (running) {
        ...
        //     Point newPoint;
        //     newPoint.x = (1 - t) * (1 - t) * (1 - t) * a0->x + 3 * (1 - t) * (1 - t) * t * c1->x + 3 * (1 - t) * t * t * c2->x + t * t * t * a3->x;
        //     newPoint.y = (1 - t) * (1 - t) * (1 - t) * a0->y + 3 * (1 - t) * (1 - t) * t * c1->y + 3 * (1 - t) * t * t * c2->y + t * t * t * a3->y;
        ...
        while (SDL_PollEvent(&event)) {
            ...
            case SDL_MOUSEBUTTONDOWN: {
                ...
                for (auto &p : points) {
                    double dist = std::sqrt((p->x - mousePosX) * (p->x - mousePosX) + (p->y - mousePosY) * (p->y - mousePosY));
                    if (dist <= 5) {
