...
public:
                               ...
                               const std::string &sourceAddress, uint16_t sourcePort)
    {
        ...
        int sockfd = socket(destAddress.find(".") == -1 ? AF_INET6 : AF_INET, SOCK_STREAM,
                            0);
        if (sockfd < 0) {
        ...
        int res = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const void *)&on,
                             sizeof(on));
        if (res < 0) {
        ...
        res = makeSockAddrAndCallFunction(destAddress, destPort,
                                       [sockfd](struct sockaddr *addr, socklen_t size) {
                                           return connect(sockfd, addr, size) >= 0 ||
                                                  errno == EINPROGRESS;
                                       });
    ...
    TCPConnInfo openListenSocket(const std::string &ipAddr, uint16_t port)
    {
        ...
        res = makeSockAddrAndCallFunction(ipAddr, port,
                                          [sockfd](struct sockaddr *addr, socklen_t size) {
                                              return bind(sockfd, addr, size);
                                          });
        ...
        if (res < 0) {
            ...
            std::cerr << "cannot bind to " << ipAddr << ":" << port << " (" << errno
    ...
    void checkForConnections(TCPConnInfo data)
    {
        ...
        while (!finish.load(std::memory_order_relaxed)) {
            ...
            if (FD_ISSET(sockfd, &set)) {
                newSockFd = makeSockAddrAndCallFunction(
                ...
                std::cerr << "New Connection, socket fd is " << newSockFd
                          << ", destIP is " << data.peerIP
                          << ", port : " << data.peerPort;
